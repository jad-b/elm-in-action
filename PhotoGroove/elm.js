...
var $author$project$PhotoGallery$initialModel = {activity: '', chosenSize: $author$project$PhotoGallery$Large, hue: 5, noise: 5, ripple: 5, status: $author$project$PhotoGallery$Loading};
var $author$project$PhotoGallery$init = function (flags) {
	var activity = function () {
		...
		if (_v0.$ === 'Ok') {
			var _float = _v0.a;
			return 'Initializing Pasta v' + $elm$core$String$fromFloat(_float);
		} else {
			return 'Error retrieving Pasta version. Oh well.';
		}
	}();
...
var $author$project$PhotoGallery$Damn = function (a) {
	return {$: 'Damn', a: a};
};
...
var $author$project$PhotoGallery$subscriptions = function (model) {
	var toMsg = function (res) {
		if (res.$ === 'Ok') {
			var msg = res.a;
			return msg;
		} else {
			var decodeError = res.a;
			return $author$project$PhotoGallery$Damn(
				$elm$json$Json$Decode$errorToString(decodeError));
		}
	};
	var msgDecoder = A2($elm$json$Json$Decode$map, $author$project$PhotoGallery$GotActivity, $elm$json$Json$Decode$string);
	var tryDecode = $elm$json$Json$Decode$decodeValue(msgDecoder);
	return $author$project$PhotoGallery$activityChanges(
		A2($elm$core$Basics$composeR, tryDecode, toMsg));
};
...
_Platform_export({'Main':{'init':$author$project$Main$main($elm$json$Json$Decode$float)(0)},'PhotoGallery':{'init':$author$project$PhotoGallery$main($elm$json$Json$Decode$value)(0)}});}(this));